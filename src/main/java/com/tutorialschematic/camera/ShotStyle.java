...
public enum ShotStyle {
    ...
    /** Весь слой в кадре, камера стоит. Опорный план, к которому можно вернуться. */
    FAR_HOLD("Дальний · статичный", 1.45, 16, 30, Movement.NONE, 0x4A90D9, 1.0, false),

    /** Слой целиком, но прицел ведёт работу — она держится ближе к центру кадра. */
    FAR_FOLLOW("Дальний · ведёт", 1.45, 16, 30, Movement.NONE, 0x50B0A0, 0.5, false),
    /** Средняя крупность, прицел точно на работе. */
    MID_FOLLOW("Средний · ведёт", 1.05, 14, 28, Movement.NONE, 0x7FD88F, 0.0, false),
    /** Вплотную к тому, что кладётся прямо сейчас. */
    NEAR_FOLLOW("Ближний · ведёт", 0.85, 12, 26, Movement.NONE, 0xC9C24A, 0.0, true),

    /** Сверху-сбоку: видно планировку и как слой ложится на предыдущие. */
    OVERVIEW_HIGH("Обзорный · сверху", 1.30, 38, 52, Movement.NONE, 0x6A8FB0, 0.6, false),
    ...
    DOLLY_IN("Наезд", 1.40, 12, 26, Movement.DOLLY_IN, 0xD9A24A, 0.0, false),
    ...
    /** Ведёт ли камера работу. Общий и опорный планы стоят неподвижно. */
