...
/**
 * Где идёт работа внутри сцены в каждый момент времени.
 *
 * <p>Камера, нацеленная в геометрический центр сцены, показывает работу в центре кадра
 * только в середине сцены: фронт постройки едет, и к концу оказывается у края. Чтобы
 * держать его в центре, нужно знать, <b>где он в какой момент</b> — этим занимается
 * {@link #sample}, единственное, что осталось от этого класса.
 *
 * <p>Раньше здесь же жило {@code windows()} — дробление слоя на временные окна, по которым
 * {@code CameraExport} решал, когда резать план. Эта роль перешла к {@link BuildAnalyzer} и
 * {@link ScenePlanner}, которые смотрят на порядок появления блоков, а не на тиковую сетку —
 * см. {@code 02_HISTORY.md} в пакете для истории вопроса.
 *
 * <p>Момент считается не по настройкам темпа, а по фактическим границам сцены в записи:
...
public final class BuildTimeline {
    ...
    public static List<FrontSample> sample(List<List<Pos>> steps, int startTick, int endTick) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // а не догоняет её и не забегает вперёд. Но первый замер — ровно на старте:
            // рез в новую сцену уже произошёл, и если первый ключевой кадр поставить в
            // середину окна, до него работу снимала бы ещё камера предыдущей сцены —
            // блоки начала сцены шли за кадром.
            int tick = i == 0 ? startTick : startTick + (int) ((i + 0.5) / count * duration);
    ...
    /** Сколько замеров на сцену: примерно по одному на {@link #TICKS_PER_SAMPLE}, но не мельче шагов. */
